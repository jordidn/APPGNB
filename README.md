...
Demo application with implementation of the two services mentioned below to retrieve the banking data on a list of products.

To obtain the data, a third-party library (Moya) has been used to retrieve the data in JSON format.
...
* You must to use Swift and any third party libraries, as you would do with any project.
* The use of design patterns, well structured code and project architecture will
be valued.
...
using an “Accept” header. Instead of using the extension try to use the “Accept” header with the right MIME type.
